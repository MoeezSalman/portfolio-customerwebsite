...
export default function NotFound() {
  ...
  return (
    <section className="relative flex min-h-dvh items-center overflow-hidden pt-28 pb-16">
      ...
      <div className="container-x relative grid items-center gap-8 lg:grid-cols-12">
        <div className="lg:col-span-7">
          <p className="font-display text-[clamp(5rem,16vw,11rem)] leading-none font-bold text-line-2">404</p>
          <h1 className="mt-4 font-display text-[clamp(1.6rem,4vw,2.6rem)] font-bold text-chalk">
            {t("notFoundTitle")}
          </h1>
          <p className="mt-4 max-w-md leading-relaxed text-fog">{t("notFoundBody")}</p>
          ...
          <ul className="mt-10 flex max-w-xl flex-wrap gap-2.5">
            {services.slice(0, 6).map((s) => (
              <li key={s.slug}>
                <Link
                  href={localePath(l, `/services/${s.slug}`)}
                  className="inline-flex rounded-full bg-ink-2 px-4 py-2 text-[0.82rem] text-mist transition-colors hover:bg-chalk hover:text-ink"
                >
                  {s.title[l]}
                </Link>
              </li>
        ...
        <Figure
          ...
          locale={l}
          className="aspect-[4/5] rounded-card lg:col-span-5"
